...
\begin{frame}
  ...
  \begin{center}
\includegraphics[height=0.65\textwidth]{figures/motivation1.eps}
\end{center}
\end{frame}
...
\begin{frame}
 ...
 \item Better analyze the impact on DOS as well as carrier concentration taking into account the non-parabolicity of the energy bands.
  \item Help experimentalists to reproduce DOS from energy-dependent DOS mass.
  \item DOS and carrier concentration obtained by full band parameterization and parabolic band approximation are compared. 
 \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  Absorption coefficient can be calculated by dielectric function directly, and thickness of material is determined by the absorption coefficient. 
  \begin{itemize}
...
DFT says that ground-state total energy ($E_{total}[\rho$]) can be determined from the ground-state charge density $\rho$ instead of many-electron wavefunction.
However, we do not have explicit form for $E_{total}[\rho$].
...
In our calculations, we took the GGA+U (d-states), which was proved to improve the effective mass, that is curvature of energy band.
???? explain LDA GGA ....
\end{frame}
...
\section{Results}
\subsection{Paper 1: parameterization of energy bands for CIGS}
...
&  m^{ab} = \pm \hbar^2 / (\partial E(\textbf{k}) / \partial{k_a}{{k_b}}).
\end{split}
\end{equation}
